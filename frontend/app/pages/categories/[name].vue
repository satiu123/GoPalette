...
import { fetchCategories, fetchPosts, isVisiblePostStatus } from '~/composables/useBlogApi'

const route = useRoute()
const { buildUrl } = useSiteSeo()
const { categoryPath, tagPath } = useBlogRoutes()

const categoryName = computed(() => decodeURIComponent(String(route.params.name || '')).trim())
const currentPage = ref(1)
const pageSize = 8

const { data: categoriesData } = await useAsyncData('category-page-categories', () => fetchCategories(1, 200))
const { data: postsData } = await useAsyncData('category-page-posts', () => fetchPosts(1, 300))

const categories = computed(() => categoriesData.value?.categories || [])
const currentCategory = computed(() => categories.value.find(item => item.name === categoryName.value) || null)

const visiblePosts = computed(() => {
  const allPosts = postsData.value?.posts || []
  return allPosts
    .filter(post => isVisiblePostStatus(post.status))
    .filter(post => post.category === categoryName.value)
})
...
const siblingCategories = computed(() => categories.value
  .filter(item => item.name !== categoryName.value)
  .slice(0, 12))
...
if (!currentCategory.value && categoryName.value) {
  throw createError({
    statusCode: 404,
    statusMessage: '分类不存在'
  })
...
const canonicalUrl = computed(() => {
  ...
  return buildUrl(`${categoryPath(categoryName.value)}${suffix}`)
})

useSeoMeta({
  title: computed(() => `${categoryName.value || '分类'} - GoPalette`),
  description: computed(() => `${categoryName.value || '分类'} 相关文章归档，共 ${visiblePosts.value.length} 篇。`)
})
