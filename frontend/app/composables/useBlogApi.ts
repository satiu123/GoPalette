...
type ApiResponse = DataRecord & { data?: DataRecord }
...
function normalizeComment(input: DataRecord): CommentInfo {
  ...
  return {
    ...
    status: typeof input.status === 'number' || typeof input.status === 'string' ? input.status : undefined,
    likeCount: String(input?.likeCount || input?.like_count || '0'),
    author: author
      ? {
          id: String(author.id || ''),
          name: String(author.name || ''),
          avatarUrl: String(author.avatarUrl || author.avatar_url || '')
        }
      : undefined,
    replyToAuthor: replyToAuthor
      ? {
          id: String(replyToAuthor.id || ''),
          name: String(replyToAuthor.name || ''),
          avatarUrl: String(replyToAuthor.avatarUrl || replyToAuthor.avatar_url || '')
        }
      : undefined,
    createdAt: String(input?.createdAt || input?.created_at || ''),
    updatedAt: String(input?.updatedAt || input?.updated_at || ''),
    replies: repliesRaw.map(reply => normalizeComment(reply as DataRecord))
  }
}

function normalizeAdminUser(input: DataRecord): AdminUserItem {
  return {
    id: String(input?.id || ''),
...
export async function fetchPosts(page = 1, pageSize = 30): Promise<{ posts: BlogPostItem[], total: number }> {
  const response = await $fetch<ApiResponse>('/api/blog/posts', {
    query: {
      page,
      pageSize
    }
...
export async function createCategory(payload: { name: string, slug?: string, description?: string }): Promise<CategoryItem | null> {
  ...
  const response = await authFetch<ApiResponse>('/api/blog/categories', {
    method: 'POST',
  ...
  const info = asRecord(categoryRecord.info || categoryRecord)
  return {
    id: String(info.id || ''),
...
export async function updateCategory(id: string, payload: { name: string, slug?: string, description?: string, updateMask?: string }): Promise<CategoryItem | null> {
  const { authFetch } = useAuth()
  const response = await authFetch<ApiResponse>(`/api/blog/categories/${id}`, {
    method: 'PATCH',
    headers: withCsrfHeaders(),
    body: {
      id,
...
export async function recordPostView(id: string): Promise<{ counted: boolean, viewCount?: number }> {
  ...
  const response = await $fetch<ApiResponse>(`/api/blog/post-views/${id}`, {
    method: 'POST',
    headers: withCsrfHeaders()
  })
...
export async function fetchPostLikeState(id: string): Promise<{ liked: boolean, likeCount: number }> {
  ...
  const { authFetch } = useAuth()
  const response = await authFetch<ApiResponse>(`/api/blog/post-likes/${id}`, {
    method: 'GET'
  })
...
export async function searchPosts(keyword: string, page = 1, pageSize = 20): Promise<{ results: SearchPostItem[], total: number, totalPages: number }> {
  ...
  const response = await $fetch<ApiResponse>('/api/blog/search', {
    query: {
      query,
      page,
      pageSize,
  ...
  const items = asRecords(response?.results || response?.items || response?.data?.results || response?.data?.items || [])
...
export async function fetchComments(postId: string, page = 1, pageSize = 50): Promise<{ comments: CommentInfo[], total: number }> {
  const response = await $fetch<ApiResponse>('/api/blog/comments', {
    query: {
      postId,
      page,
      pageSize
  ...
  const commentList = asRecords(response?.comments || response?.data?.comments || [])
  return {
    comments: commentList.map(item => normalizeComment(item)),
    total: Number(response?.total || response?.data?.total || 0)
  }
}
...
export async function createComment(payload: { postId: string, content: string, parentId?: string }): Promise<CommentInfo | null> {
  const { authFetch } = useAuth()
  const response = await authFetch<ApiResponse>('/api/blog/comments', {
    method: 'POST',
    headers: withCsrfHeaders(),
    body: payload
  })
  ...
  return comment ? normalizeComment(comment as DataRecord) : null
}

export async function deleteComment(id: string): Promise<boolean> {
  const { authFetch } = useAuth()
  const response = await authFetch<ApiResponse>(`/api/blog/comments/${id}`, {
    method: 'DELETE',
    headers: withCsrfHeaders()
  })
...
export async function fetchAdminUsers(page = 1, pageSize = 20): Promise<{ users: AdminUserItem[], total: number }> {
  const { authFetch } = useAuth()
  const response = await authFetch<ApiResponse>('/api/user/users', {
    method: 'GET',
    query: {
      page,
      pageSize
  ...
  const users = (response?.users || response?.data?.users || []) as DataRecord[]
  return {
...
export async function createAdminUser(payload: {
  ...
  return user ? normalizeAdminUser(user as DataRecord) : null
}
...
export async function updateAdminUser(id: string, payload: {
  ...
  const { authFetch } = useAuth()
  const response = await authFetch<ApiResponse>(`/api/user/users/${id}`, {
    method: 'PATCH',
    query: {
...
export async function createPost(payload: {
  ...
  const { authFetch } = useAuth()
  const response = await authFetch<ApiResponse>('/api/blog/posts', {
    method: 'POST',
    headers: withCsrfHeaders(),
    body: payload
  })
...
export async function updatePost(id: string, payload: {
  ...
  const { authFetch } = useAuth()
  const response = await authFetch<ApiResponse>(`/api/blog/posts/${id}`, {
    method: 'PATCH',
    headers: withCsrfHeaders(),
    body: {
      id,
...
export async function createTag(payload: { name: string, slug?: string }): Promise<TagItem | null> {
  const { authFetch } = useAuth()
  const response = await authFetch<ApiResponse>('/api/blog/tags', {
    method: 'POST',
    headers: withCsrfHeaders(),
    body: payload
  })
  ...
  const info = asRecord(tagRecord.info || tagRecord)
...
export async function updateTag(id: string, payload: { name: string, slug?: string, updateMask?: string }): Promise<TagItem | null> {
  const { authFetch } = useAuth()
  const response = await authFetch<ApiResponse>(`/api/blog/tags/${id}`, {
    method: 'PATCH',
    headers: withCsrfHeaders(),
    body: {
      id,
